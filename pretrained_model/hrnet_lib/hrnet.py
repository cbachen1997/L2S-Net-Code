...
class HRnet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # tic = time.time()
        feat = F.interpolate(x, size=(H, W), mode='bilinear', align_corners=True).squeeze()
        feat = feat.transpose(1, 2).transpose(2, 0).transpose(1, 0)
        # print('Feat Interpolation time: {}'.format(toc-tic))

        x = self.last_layer(x)
        # tic = time.time()
        x = F.interpolate(x, size=(H, W), mode='bilinear', align_corners=True)
        # toc = time.time()
        # print('Class Interpolation time: {}'.format(toc-tic))
        return x, feat
        # return x
